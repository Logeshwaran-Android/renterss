...
class CompletedBookingFragment : Fragment(),MyBookingsAdapter.MyBookingsClickListener {
    ...
    private fun callGetBookings(){
        myBookingsViewModel.callGetCompletedBookingList(mPage).observe(viewLifecycleOwner, Observer<MyTripsResponse> {
        ...
        if(resultCode == RESULT_OK){
            if(requestCode == Iconstants.REVIEW_REQUEST_CODE){
                mSelectedPosition?.let {
                    mBookingList[mSelectedPosition!!].review_status = "1"
