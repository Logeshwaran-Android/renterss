...
import com.rent.renters.app.ui.myBookings.InvoiceActivity
...
class                                                                                                                              TotalReservationsFragment : Fragment() ,View.OnClickListener, CustomRecyclerViewAdapter.CustomItemClickListener,MyReservationsAdapter.MyReservationClickListener, Util.BottomApproveClickListener  {
    ...
    private var mReservationItem :MyReservationListData ?= null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
     ...
     fun initViewModel() {
        ...
        callGetTotalReservations()
    }
   private fun callGetTotalReservations() {
       myReservationViewModel.callGetTotalReservationList(mPage).observe(viewLifecycleOwner, Observer<MyReservationResponse> {
    ...
    override fun onApproveClick(message: String,isFrom: String) {
        if(isFrom.equals(Iconstants.PRE_APPROVE,true)) {
            if (mReservationItem != null) {
                myReservationViewModel.callApproveDeclineRequest(message, mSessionManager.getUserDetails().id, mReservationItem?.userid!!, mReservationItem?.booking_no!!, "Accepted").observe(this, Observer {
                    (context as BaseActivity).baseViewModel.rentersLoader.postValue(false)
                    (context as BaseActivity).baseViewModel.rentersError.postValue(it.message)
                    mSelectedPosition?.let {
                        mReservationList[mSelectedPosition!!].booking_status = Iconstants.ACCEPTED
                        myReservationsAdapter!!.notifyItemChanged(mSelectedPosition!!)
                    }
                   // callGetTotalReservations()

                })
            }
        }else  if(isFrom.equals(Iconstants.DECLINE,true)) {
            if (mReservationItem != null) {
                myReservationViewModel.callApproveDeclineRequest(message, mSessionManager.getUserDetails().id, mReservationItem?.userid!!, mReservationItem?.booking_no!!, "Declined").observe(this, Observer {
                    (context as BaseActivity).baseViewModel.rentersLoader.postValue(false)
                    (context as BaseActivity).baseViewModel.rentersError.postValue(it.message)
                    mSelectedPosition?.let {
                        mReservationList[mSelectedPosition!!].booking_status = Iconstants.DECLINED
                        myReservationsAdapter!!.notifyItemChanged(mSelectedPosition!!)
                    }
                  //  callGetTotalReservations()
                })
