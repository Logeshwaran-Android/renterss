...
class TotalBookingsFragment : Fragment(),MyBookingsAdapter.MyBookingsClickListener  {
    ...
    private fun initView() {
        if(arguments!=null){
            if(arguments?.containsKey(Iconstants.IS_FROM)!!){
                ...
                if(mIsFrom == Iconstants.BOOKING_PAGE){
                    val invoiceIntent = Intent(context,InvoiceActivity::class.java)
                    invoiceIntent.putExtra(Iconstants.BOOKING_NO,mBookingNo)
    ...
    private fun callGetBookings(){
        myBookingsViewModel.callGetTotalBookingList(mPage).observe(viewLifecycleOwner, Observer<MyTripsResponse> {
    ...
    override fun bookingClick(item: MyTripsListData, isFrom: String,position:Int) {
        ...
        when(isFrom){
            ...
            Iconstants.ADD_REVIEW->{
                ...
                startActivityForResult(addReviewIntent,Iconstants.REVIEW_REQUEST_CODE)
            }
            ...
            Iconstants.CANCEL_BOOKING ->{
                val viewReviewIntent = Intent(context, CancelBookingActivity::class.java)
                viewReviewIntent.putExtra(Iconstants.BOOKING_NO,item.booking_no)
                startActivity(viewReviewIntent)
            }
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(resultCode == RESULT_OK){
            if(requestCode == Iconstants.REVIEW_REQUEST_CODE){
                mSelectedPosition?.let {
                    mBookingList[mSelectedPosition!!].review_added = "1"
                    myBookingAdapter!!.notifyItemChanged(mSelectedPosition!!)
                }
